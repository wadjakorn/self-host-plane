...
│   ├── plane.env.example   # tracked: sanitized template for plane-app/plane.env
│   └── plane-app/          # generated by setup.sh
│       ├── docker-compose.yaml   # tracked
│       └── plane.env        # gitignored: real config + secrets
└── plane-mcp/              # Plane MCP server (HTTP transport)
    ├── docker-compose.yml
    ├── Dockerfile
    ├── .env.example        # tracked template
    └── .env                # gitignored: MCP config (instance URL, port)
```

> **Git note:** secrets never leave your machine. `plane-app/plane.env` and `plane-mcp/.env`
> are gitignored; the `*.env.example` files are the tracked templates. The Plane
> `docker-compose.yaml` is tracked (it holds only env references, no secrets).
...
On a fresh clone, generate the stack first (`plane-app/` is not committed):

```bash
cd plane-selfhost
./setup.sh install     # downloads compose + pulls images, creates plane-app/plane.env
# compare the generated env against the template and set secrets:
#   diff plane.env.example plane-app/plane.env
./setup.sh start
```
